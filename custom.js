    ...
    .then((data) => {
      ...
      for (user of data.data) {
        ...
        table.innerHTML += `<tr>
                ...
                <td>
                  <a class="btn btn-warning" onclick="updateUser(${user.id})">Güncelle</a>
                  <a class="btn btn-danger" onclick="deleteUser(${user.id})">Sil</a>
                </td>
              </tr>`;
      }
    });
...
function createUser() {
  ...
  fetch("https://reqres.in/api/users", {
    ...
    .then((data) => {
      ...
      <td>
        <a class="btn btn-warning" onclick="updateUser(${data.id})">Güncelle</a>
        <a class="btn btn-danger" onclick="deleteUser(${data.id})">Sil</a>
      </td>
    </tr>`;
    })
    .catch((error) => {
...
function updateUser(id) {
  ...
  let data = {
    first_name:
      document.getElementById("first_name_" + id).value || "Değer Yok",
    last_name: document.getElementById("last_name_" + id).value || "Değer Yok",
    email: document.getElementById("email_" + id).value || "Değer Yok",
  };
  console.log(data);
  fetch("https://reqres.in/api/users", {
    method: "PUT",
